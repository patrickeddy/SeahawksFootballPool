...
.controller('DashCtrl', function ($scope) {
    ...
    if (game != null)
        for (var i = 0; i < 4; i++) {
            $scope.winners.push(game['qtr' + (i + 1) + 'Winner']);
        }
})
...
.controller('ResultsCtrl', function ($scope, $location, $timeout) {
    ...
    $scope.generateWinners = function () {
        ...
        for (var count = 0; count < 4; count++) {
            ...
            $scope.game["qtr" + (count + 1) + "Winner"].score = scores[count][0] + " - " + scores[count][1];
        }
